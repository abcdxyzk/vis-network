...
class Edge {
  ...
  static parseOptions(
    ...
    if (isValidLabel(newOptions.labelFrom)) {
  ...
  drawLabel(ctx, viaNode) {
    ...
    if (this.options.labelTo !== undefined) {
       ...
       this.updateLabelModule();
       this._drawLabel(0.9, ctx, viaNode);
       this.options.label = saved;
       this.updateLabelModule();
    }
  }

  _drawLabel(offset, ctx, viaNode) {
      // set style
      ...
      if (node1.id != node2.id) {
        ...
        point = this.edgeType.getPoint(offset, viaNode);
        ctx.save();
	...
	// 把labelFrom, labelTo 移到两个端点上
        var www = this.labelModule.lines[0].width;
        if (www < 80)
          www = www / 1.5;
        else if (www < 100)
          www = www / 1.7;
        ...
        if (offset < 0.3) {
          point = viaNode[0];
          point.x = point.x - (this.to.x - this.from.x) * this.options.smooth.roundness + www;
        }
        if (offset > 0.8) {
          point = viaNode[1];
          point.x = point.x + (this.to.x - this.from.x) * this.options.smooth.roundness - www;
...
*/
        ...
        var pct = offset < 0.5 ? 0.125*4 // i.e. labelFrom
                : offset > 0.5 ? 0.125*7 // i.e. labelTo
                : 0.125;                 // i.e. label
  ...
  isOverlappingWith(obj) {
    if (this.connected) {
      const distMax = 10;
      const xFrom = this.from.x + this.from.shape.width / 2;
      const yFrom = this.from.y + this.options.ei;
      const xTo = this.to.x - this.to.shape.width / 2;
      const yTo = this.to.y;
      const xObj = obj.left;
      const yObj = obj.top;
